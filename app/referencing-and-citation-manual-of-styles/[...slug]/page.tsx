...
export function generateStaticParams() {
  return getAllStaticSlugs("referencing-and-citation-manual-of-styles").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const path = `/referencing-and-citation-manual-of-styles/${slug.join("/")}/`;
  const proxied = getStaticContent("referencing-and-citation-manual-of-styles", slug);
...
export default async function ReferencingSlugPage({ params }: Props) {
  ...
  return (
    <>
      <BreadcrumbSchema items={[{ name: "Home", url: "/" }, { name: "Referencing Guides", url: "/referencing-and-citation-manual-of-styles/" }, { name: title , url: `https://www.tutorsindia.com/referencing-and-citation-manual-of-styles/${slug.join("/")}/` }]} />
      {/* Hero */}
        ...
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          <div style={{ fontSize: "0.82rem", color: "#a0b8e0", marginBottom: "12px", display: "flex", flexWrap: "wrap", gap: "4px" }}>
            {crumbs.map((crumb, i) => {
              ...
              // Only Home and the Referencing hub are real pages.
              return (
