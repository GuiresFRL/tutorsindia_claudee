...
interface Props {
  params: Promise<{ category: string; slug: string; postSlug: string }>;
}

// Some academy posts carry a legacy two-segment category path in Payload
// (e.g. assignment-writing/economics-finance/<slug>) that predates the
// site's flat single-category URL structure (/academy/<category>/<slug>/).
// Rather than 404 or redirect those away from the URL Payload considers
// correct, this route serves the same content directly at whatever
// two-segment path is requested — no redirect, no placeholder.
export const revalidate = 60;

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { category, slug, postSlug } = await params;
  const post = await getPayloadPostBySlug("academy", postSlug);
  ...
  return {
    ...
    alternates: { canonical: `https://www.tutorsindia.com/academy/${category}/${slug}/${postSlug}/` },
    openGraph: {
      title,
      description,
      type: "article",
      ...
      url: `https://tutorsindia.com/academy/${category}/${slug}/${postSlug}/`,
...
export default async function AcademyNestedCategoryPostPage({ params }: Props) {
  ...
  return (
    <>
      <BreadcrumbSchema items={[{ name: "Home", url: "/" }, { name: "Academy", url: "/academy/" }, { name: post.title , url: `https://www.tutorsindia.com/academy/${category}/${slug}/${postSlug}/` }]} />
      {/* ── HERO ── */}
      ...
      {/* Content styles — identical to the single-category academy post page */}
      <style>{`
