...
export async function generateMetadata(): Promise<Metadata> {
  const page = await getTIPageById(14476);
  const desc = page ? stripTIHtml(page.excerpt.rendered, 160) : "";
  return {
    title: "Essays — University Essay Examples & Samples | Tutors India",
    description: desc || "Browse university essay examples demonstrating proper structure, research methods, and citation styles for academic writing.",
  ...
  return (
    <>
      ...
      <section style={{ background: "linear-gradient(135deg,#1a2a6c 0%,#2563b0 100%)", color: "#fff", padding: "52px 20px 44px" }}>
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          ...
          <h1 style={{ fontFamily: "Merriweather,serif", fontSize: "clamp(1.5rem,3vw,2.2rem)", marginBottom: "10px" }}>
            {page ? <span dangerouslySetInnerHTML={{ __html: page.title.rendered }} /> : "Essays"}
          </h1>
          <p style={{ color: "#c5d5f0", fontSize: "0.97rem", maxWidth: "680px" }}>
            University essay examples demonstrating proper structure, research methods, and citation styles for academic writing.
          </p>
        </div>
      </section>
      ...
      <section style={{ background: "linear-gradient(135deg,#1a2a6c 0%,#2563b0 100%)", color: "#fff", padding: "48px 20px", textAlign: "center" }}>
        <h2 style={{ fontFamily: "Merriweather,serif", fontSize: "1.4rem", fontWeight: 700, marginBottom: "10px" }}>Need Help With Your Essay?</h2>
        <p style={{ color: "#c5d5f0", fontSize: "0.93rem", marginBottom: "20px" }}>Our PhD-qualified experts can help you write, edit, or proofread any essay.</p>
