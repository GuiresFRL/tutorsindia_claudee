...
export function generateStaticParams() {
  return getAllStaticSlugs("marketing").map((slug) => ({ slug }));
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params;
  const path = `/marketing/${slug.join("/")}/`;
  const proxied = getStaticContent("marketing", slug);
...
export default async function MarketingSlugPage({ params }: Props) {
  ...
  const crumbs = [
    ...
    ...slug.map((segment, i) => ({
      ...
      href: `/marketing/${slug.slice(0, i + 1).join("/")}/`,
    })),
  ];

  return (
    <>
      <BreadcrumbSchema items={[{ name: "Home", url: "/" }, { name: "Marketing", url: "/marketing/" }, { name: title , url: `https://www.tutorsindia.com/marketing/${slug.join("/")}/` }]} />
      {/* Hero */}
        ...
        <div style={{ maxWidth: "1100px", margin: "0 auto" }}>
          <div style={{ fontSize: "0.82rem", color: "#a0b8e0", marginBottom: "12px", display: "flex", flexWrap: "wrap", gap: "4px" }}>
            {crumbs.map((crumb, i) => {
              ...
              // Only Home and the Marketing hub are real pages.
              return (
          ...
          <p style={{ color: "#c5d5f0", fontSize: "0.96rem" }}>Marketing — Academic Sample Work</p>
        </div>
      </section>
        ...
        {proxied.content && proxied.content.trim().length > 100 ? (
          ...
          <div style={{ background: "#fff", border: "1px solid #dde2ef", borderRadius: "10px", padding: "32px" }}>
            ...
            <div style={{ display: "flex", gap: "12px", marginTop: "20px" }}>
              ...
              <Link href="/marketing/" style={{ padding: "10px 24px", border: "1.5px solid #1a2a6c", color: "#1a2a6c", borderRadius: "5px", fontWeight: 600 }}>← Marketing</Link>
            </div>
          </div>
        )}
      </div>
